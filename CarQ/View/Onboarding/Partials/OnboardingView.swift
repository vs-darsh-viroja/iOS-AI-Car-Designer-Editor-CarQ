...
struct OnboardingView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ZStack {
                ...
                .padding(.top, ScaleUtility.scaledSpacing(60))
                
                VStack(spacing: ScaleUtility.scaledSpacing(5)) {
                    
                    Text(title)
                        .font(FontManager.ChakraPetchSemiBoldFont(size: .scaledFontSize(48)))
                ...
                .padding(.top, ScaleUtility.scaledSpacing(86))
            }
        ...
        .ignoresSafeArea(.all)
        .onAppear {
            print("OnboardingView \(screenIndex) appeared, currentScreenIndex: \(currentScreenIndex)")
            // Immediately reset to off-screen position
            resetToOffScreen()
            
            // Only animate if this is the current screen
        ...
        .onChange(of: currentScreenIndex) { oldIndex, newIndex in
            print("Screen changed from \(oldIndex) to \(newIndex), my index is \(screenIndex)")
            
            if newIndex == screenIndex {
                // Reset immediately when this screen becomes active
                resetToOffScreen()
    ...
    private func startAnimation() {
        print("Starting animation for screen \(screenIndex)")
