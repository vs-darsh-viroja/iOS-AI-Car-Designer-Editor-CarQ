...
struct ReplaceObjectView: View {
        ...
        ZStack(alignment: .bottom) {
            ...
            GenerateButtonView(isDisabled: false, action: {
                if brushedAreas.isEmpty {
                    activeAlert = .processingError(message: "Please brush the areas where you want to replace an object")
            ...
            ProcessingView(
                ...
                onAppear: {
                    Task {
                        ...
                        viewModel.currentSource = "ReplaceObjectView"
                        viewModel.currentPrompt = prompt

                        // Build prompt (requires you to add this helper)
                        let finalPrompt = PromptBuilder.buildReplaceObjectPrompt(
                            userPrompt: prompt,
    ...
    private func mainBrushInterface() -> some View {
        VStack(spacing: 20) {
            ...
            PromptView(screen: "ReplaceObject", prompt: $prompt, isInputFocused: $searchFocused)
        }
    }
